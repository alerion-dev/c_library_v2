...
#define MAVLINK_MSG_ID_PAYLOAD_STATUS 45002
...
#define MAVLINK_MSG_ID_PAYLOAD_STATUS_LEN 1
#define MAVLINK_MSG_ID_PAYLOAD_STATUS_MIN_LEN 1
#define MAVLINK_MSG_ID_45002_LEN 1
#define MAVLINK_MSG_ID_45002_MIN_LEN 1

#define MAVLINK_MSG_ID_PAYLOAD_STATUS_CRC 30
#define MAVLINK_MSG_ID_45002_CRC 30
...
#define MAVLINK_MESSAGE_INFO_PAYLOAD_STATUS { \
    ...
    "PAYLOAD_STATUS", \
    1, \
    {  { "status", NULL, MAVLINK_TYPE_UINT8_T, 0, 0, offsetof(mavlink_payload_status_t, status) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_PAYLOAD_STATUS { \
...
{
    return mavlink_msg_payload_status_pack(system_id, component_id, msg, payload_status->status);
}
...
{
    return mavlink_msg_payload_status_pack_chan(system_id, component_id, chan, msg, payload_status->status);
}
...
static inline void mavlink_msg_payload_status_send(mavlink_channel_t chan, uint8_t status)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    char buf[MAVLINK_MSG_ID_PAYLOAD_STATUS_LEN];
    _mav_put_uint8_t(buf, 0, status);
...
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
    mavlink_msg_payload_status_send(chan, payload_status->status);
#else
 ...
 */
static inline void mavlink_msg_payload_status_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t status)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
